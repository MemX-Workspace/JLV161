...
struct port_wakeup port4 = {
	...
	.iomap      = TCFG_EAR_DETECT_DET_IO,                             //唤醒口选择
};
#endif
#endif
...
//                            外置触摸和入耳检测供电配置                           //
//*********************************************************************************//
#if TCFG_EXTERNAL_TOUCH_KEY_ENABLE
...
static void pwr_set_external_touch(u8 high_low)
{
	...
	gpio_set_pull_up(TCFG_EXTERNAL_TOUCH_KEY_EN_PORT, 1);
	gpio_set_pull_down(TCFG_EXTERNAL_TOUCH_KEY_EN_PORT, 0);
	gpio_set_direction(TCFG_EXTERNAL_TOUCH_KEY_EN_PORT, 0);
	gpio_set_output_value(TCFG_EXTERNAL_TOUCH_KEY_EN_PORT, high_low);
}
//仓储模式的关闭外置触摸
void external_touch_key_disable(void)
{
	pwr_set_external_touch(0);
}
#endif
...
#if TCFG_EAR_DETECT_ENABLE
	ear_detect_init(&ear_detect_cfg);
#endif
...
void board_set_soft_poweroff(void)
{
    ...
    //关闭HPVDD外接1.85V电源
	gpio_set_direction(TCFG_AUDIO_HPVDD_PORT,0);
	gpio_set_output_value(TCFG_AUDIO_HPVDD_PORT,0);
#endif

#if TCFG_EXTERNAL_TOUCH_KEY_ENABLE
/* 外置触摸和入耳检测的电源保持 */
    soff_gpio_protect(TCFG_EXTERNAL_TOUCH_KEY_POWER_PORT);
    soff_gpio_protect(TCFG_EXTERNAL_TOUCH_KEY_EN_PORT);
#endif

#if TCFG_EXTERN_STORAGE_MODE_ENABLE
/* BOAT_PL在softoff的时候也保持高,不然会触发模块断电 */
